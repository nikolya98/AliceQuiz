...
def create_thematic_db():
    ...
    names = get_file_names('data_bases/thematic')
    ...
    for name in names:
        ...
        with open(f'data_bases/thematic/{name}.txt', 'r', encoding='utf-8') as f:
            data = f.readline()
            while data:
...
def create_difficult_db():
    names = get_file_names('data_bases/difficult')
    ...
    for name in names:
        cursor.execute(f"""CREATE TABLE IF NOT EXISTS {name}"""
                       + """ (question text, answer text, UNIQUE (question))""")

        with open(f'data_bases/difficult/{name}.txt', 'r', encoding='utf-8') as f:
            data = f.readline()
            while data:
                ins_data = tuple(i.strip() for i in data.split('*'))
                cursor.execute(f"""INSERT OR IGNORE INTO {name} VALUES {ins_data}""")
